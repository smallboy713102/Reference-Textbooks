\documentclass[11pt]{article}
    ...
    % Document parameters
    % Document title
    \title{C1\_W3\_Lab03\_Decision\_Boundary\_Soln}
...
\begin{document}
    ...
    \hypertarget{optional-lab-logistic-regression-decision-boundary}{%
\section{Optional Lab: Logistic Regression, Decision
Boundary}\label{optional-lab-logistic-regression-decision-boundary}}
...
\PY{k+kn}{from} \PY{n+nn}{lab\PYZus{}utils\PYZus{}common} \PY{k+kn}{import} \PY{n}{plot\PYZus{}data}\PY{p}{,} \PY{n}{sigmoid}\PY{p}{,} \PY{n}{draw\PYZus{}vthresh}
...
\texttt{X} is a numpy array which has 6 training examples, each with two
features - The output variable \texttt{y} is also a numpy array with 6
examples, and \texttt{y} is either \texttt{0} or \texttt{1}
...
\PY{n}{y} \PY{o}{=} \PY{n}{np}\PY{o}{.}\PY{n}{array}\PY{p}{(}\PY{p}{[}\PY{l+m+mi}{0}\PY{p}{,} \PY{l+m+mi}{0}\PY{p}{,} \PY{l+m+mi}{0}\PY{p}{,} \PY{l+m+mi}{1}\PY{p}{,} \PY{l+m+mi}{1}\PY{p}{,} \PY{l+m+mi}{1}\PY{p}{]}\PY{p}{)}\PY{o}{.}\PY{n}{reshape}\PY{p}{(}\PY{o}{\PYZhy{}}\PY{l+m+mi}{1}\PY{p}{,}\PY{l+m+mi}{1}\PY{p}{)} 
\end{Verbatim}
\end{tcolorbox}
...
\PY{n}{plot\PYZus{}data}\PY{p}{(}\PY{n}{X}\PY{p}{,} \PY{n}{y}\PY{p}{,} \PY{n}{ax}\PY{p}{)}
...
plotting its decision boundary
    ...
    \hypertarget{refresher-on-logistic-regression-and-decision-boundary}{%
\subsubsection{Refresher on logistic regression and decision
boundary}\label{refresher-on-logistic-regression-and-decision-boundary}}
...
\item
  ...
  where \(g(z)\) is known as the sigmoid function and it maps all input
  values to values between 0 and 1:

  \(g(z) = \frac{1}{1+e^{-z}}\tag{2}\) and
  \(\mathbf{w} \cdot \mathbf{x}\) is the vector dot product:

  \[\mathbf{w} \cdot \mathbf{x} = w_0 x_0 + w_1 x_1\]
\item
  We interpret the output of the model (\(f_{\mathbf{w},b}(x)\)) as the
  probability that \(y=1\) given \(\mathbf{x}\) and parameterized by
  \(\mathbf{w}\) and \(b\).
\item
  Therefore, to get a final prediction (\(y=0\) or \(y=1\)) from the
  logistic regression model, we can use the following heuristic -
...
\begin{Verbatim}[commandchars=\\\{\}]
\PY{c+c1}{\PYZsh{} Plot sigmoid(z) over a range of values from \PYZhy{}10 to 10}
\PY{n}{z} \PY{o}{=} \PY{n}{np}\PY{o}{.}\PY{n}{arange}\PY{p}{(}\PY{o}{\PYZhy{}}\PY{l+m+mi}{10}\PY{p}{,}\PY{l+m+mi}{11}\PY{p}{)}
...
\PY{n}{ax}\PY{o}{.}\PY{n}{plot}\PY{p}{(}\PY{n}{z}\PY{p}{,} \PY{n}{sigmoid}\PY{p}{(}\PY{n}{z}\PY{p}{)}\PY{p}{,} \PY{n}{c}\PY{o}{=}\PY{l+s+s2}{\PYZdq{}}\PY{l+s+s2}{b}\PY{l+s+s2}{\PYZdq{}}\PY{p}{)}
...
\item
  ...
  \(z = \mathbf{w} \cdot \mathbf{x} + b\). Therefore,

  if \(\mathbf{w} \cdot \mathbf{x} + b >= 0\), the model predicts
  \(y=1\)

  if \(\mathbf{w} \cdot \mathbf{x} + b < 0\), the model predicts \(y=0\)
\end{itemize}

\hypertarget{plotting-decision-boundary}{%
\subsubsection{Plotting decision
boundary}\label{plotting-decision-boundary}}
...
\(-3 + x_0+x_1 = 0\), which is equivalent to \(x_1 = 3 - x_0\).
...
\PY{n}{ax}\PY{o}{.}\PY{n}{plot}\PY{p}{(}\PY{n}{x0}\PY{p}{,}\PY{n}{x1}\PY{p}{,} \PY{n}{c}\PY{o}{=}\PY{l+s+s2}{\PYZdq{}}\PY{l+s+s2}{b}\PY{l+s+s2}{\PYZdq{}}\PY{p}{)}
\PY{n}{ax}\PY{o}{.}\PY{n}{axis}\PY{p}{(}\PY{p}{[}\PY{l+m+mi}{0}\PY{p}{,} \PY{l+m+mi}{4}\PY{p}{,} \PY{l+m+mi}{0}\PY{p}{,} \PY{l+m+mf}{3.5}\PY{p}{]}\PY{p}{)}

\PY{c+c1}{\PYZsh{} Fill the region below the line}
\PY{n}{ax}\PY{o}{.}\PY{n}{fill\PYZus{}between}\PY{p}{(}\PY{n}{x0}\PY{p}{,}\PY{n}{x1}\PY{p}{,} \PY{n}{alpha}\PY{o}{=}\PY{l+m+mf}{0.2}\PY{p}{)}
...
\item
  ...
  \(x_0 + x_1 - 3 = 0\) and it should intersect the x1 axis at 3 (if we
  set \(x_1\) = 3, \(x_0\) = 0) and the x0 axis at 3 (if we set \(x_1\)
  = 0, \(x_0\) = 3).
\item
  The shaded region represents \(-3 + x_0+x_1 < 0\). The region above
  the line is \(-3 + x_0+x_1 > 0\).
\item
  Any point in the shaded region (under the line) is classified as
  \(y=0\). Any point on or above the line is classified as \(y=1\). This
  line is known as the ``decision boundary''.
\end{itemize}

As we've seen in the lectures, by using higher order polynomial terms
(eg: \(f(x) = g( x_0^2 + x_1 -1)\), we can come up with more complex
